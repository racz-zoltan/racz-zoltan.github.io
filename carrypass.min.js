const MAX_CALLS_PER_MINUTE=5;const RATE_LIMIT_WINDOW=60*1e3;let callTimes=[];function isRateLimited(){const currentTime=Date.now();callTimes=callTimes.filter(time=>currentTime-time<RATE_LIMIT_WINDOW);return callTimes.length>=MAX_CALLS_PER_MINUTE}function updateCounter(){callTimes.push(Date.now())}const MAX_ATTEMPTS_PER_MIN=5;const RATE_LIMIT_WINDOW_MS=60*1e3;let unlockAttempts=[];function isViewerRateLimited(){const now=Date.now();unlockAttempts=unlockAttempts.filter(ts=>now-ts<RATE_LIMIT_WINDOW_MS);return unlockAttempts.length>=MAX_ATTEMPTS_PER_MIN}function recordViewerAttempt(){unlockAttempts.push(Date.now())}async function confirmModal(key){return new Promise(resolve=>{const message=t(key);showConfirmation(message,result=>resolve(result))})}const translations={};async function loadLanguage(lang){if(translations[lang])return;try{const response=await fetch(`./lang/${lang}.json`);if(!response.ok)throw new Error(`Failed to load ${lang}.json`);translations[lang]=await response.json()}catch(error){console.error(`Error loading language file: ${error}`)}}let vault={};function enforceDigitInput(inputId){const inputElement=document.getElementById(inputId);if(!inputElement)return;inputElement.addEventListener("input",function(){this.value=this.value.replace(/[^0-9]/g,"")})}enforceDigitInput("iterationCount");function generateRandomUsername(webAddress,salt,length){const charset=PASSWORDCHARSETS.lowerAlpha;const input=webAddress+"::"+":pwd:"+"::"+salt;const keySize=Math.ceil(length/4);const hash=CryptoJS.PBKDF2(input,salt,{keySize:keySize,iterations:5e3,hasher:CryptoJS.algo.SHA256});let username="";const words=hash.words;for(let i=0;i<length;i++){const byte=words[Math.floor(i/4)]>>24-i%4*8&255;username+=charset[byte%charset.length]}return username}async function generateUsername(){let webAddress=document.getElementById("webAddress").value;let iterationCount=document.getElementById("iterationCount").value;const salt=generateWithMaster(webAddress);let length=14;let key=generateRandomUsername(webAddress,iterationCount,salt,length);document.getElementById("username").value=key}let generatedUsernames=[];let generatedPasswords=[];let currentPasswordIndex=0;let currentUsernameIndex=0;function updateCredentialPreviews(){const usernameField=document.getElementById("usernamePreview");const passwordField=document.getElementById("passwordPreview");if(generatedUsernames.length>0){usernameField.value=generatedUsernames[currentUsernameIndex];document.getElementById("usernamePreview").value=generatedUsernames[currentUsernameIndex];document.getElementById("usernameIndexLabel").textContent=`(${currentUsernameIndex+1}/6)`}if(generatedPasswords.length>0){passwordField.value=generatedPasswords[currentPasswordIndex];document.getElementById("passwordPreview").value=generatedPasswords[currentPasswordIndex];document.getElementById("passwordIndexLabel").textContent=`(${currentPasswordIndex+1}/6)`}}function cycleUsername(up){if(up){currentUsernameIndex=(currentUsernameIndex+1)%generatedUsernames.length}else{currentUsernameIndex=(currentUsernameIndex-1+generatedUsernames.length)%generatedUsernames.length}updateCredentialPreviews()}function cyclePassword(up){if(up){currentPasswordIndex=(currentPasswordIndex+1)%generatedPasswords.length}else{currentPasswordIndex=(currentPasswordIndex-1+generatedPasswords.length)%generatedPasswords.length}updateCredentialPreviews()}function showUsernameDropdown(choices,input){const container=document.getElementById("usernameOptions");container.innerHTML="";container.classList.remove("hidden");choices.forEach((name,i)=>{const opt=document.createElement("div");opt.textContent=name;opt.className="username-suggestion";opt.addEventListener("click",()=>{input.value=name;container.classList.add("hidden")});container.appendChild(opt)})}async function generateUsernameSalt(appMasterHex,webAddress){const keyData=new Uint8Array(appMasterHex.match(/.{1,2}/g).map(b=>parseInt(b,16)));const cryptoKey=await crypto.subtle.importKey("raw",keyData,{name:"HMAC",hash:"SHA-256"},false,["sign"]);const signature=await crypto.subtle.sign("HMAC",cryptoKey,(new TextEncoder).encode("username::"+webAddress));return btoa(String.fromCharCode(...new Uint8Array(signature))).slice(0,32)}function structureUsernameCV(rawKey){const consonants="bcdfghjklmnpqrstvwxyz";const vowels="aeiou";const syllables=[];for(let i=0;i<10;i+=2){const c=consonants[rawKey.charCodeAt(i)%consonants.length];const v=vowels[rawKey.charCodeAt(i+1)%vowels.length];const syllableIndex=i/2;const consonant=syllableIndex===0||syllableIndex===2?c.toUpperCase():c;syllables.push(consonant+v)}return syllables.join("")}function capitalize(word){return word.charAt(0).toUpperCase()+word.slice(1)}function updateGeneratedPasswords(passwords){generatedPasswords=passwords;if(typeof currentPasswordIndex!=="number"||isNaN(currentPasswordIndex)){currentPasswordIndex=0}updateCredentialPreviews()}function updateGeneratedUsernames(usernames){generatedUsernames=usernames;if(typeof currentUsernameIndex!=="number"||isNaN(currentUsernameIndex)){currentUsernameIndex=0}updateCredentialPreviews()}async function generateUsernames(){const webAddress=document.getElementById("webAddress").value.trim();const salt=await generateUsernameSalt(appMaster,webAddress);const baseLength=14;const usernames=[];for(let i=0;i<6;i++){const variantSalt=salt+"::"+i;const rawKey=generateRandomUsername(webAddress,variantSalt,baseLength);const formattedUsername=structureUsernameCV(rawKey);usernames.push(formattedUsername)}updateGeneratedUsernames(usernames)}function showAlert(message,type="info",container=document.body){const alert=document.createElement("div");alert.className=`custom-alert custom-alert-${type}`;alert.innerHTML=`
    <span>${message}</span>
    <button class="custom-alert-close" onclick="this.parentElement.remove()">×</button>
  `;container.prepend(alert);setTimeout(()=>{alert.remove()},4e3)}function generateWithMaster(webAddress){var input=webAddress+":pwd:";var base32Chars="ABCDEFGHIJKLMNOPQRSTUVWXYZ234567";while(input.length<32){input+=input}input=input.slice(0,32);var base64=btoa(input);var result="";for(var i=0;i<base64.length;i++){var char=base64.charAt(i);if(char!=="="){var index=char.charCodeAt(0);if(index===43)index=62;else if(index===47)index=63;result+=base32Chars[index>>2];result+=base32Chars[(index&3)<<3|base64.charCodeAt(++i)>>5];result+=base32Chars[base64.charCodeAt(i)&31]}}result=result.slice(0,32);document.getElementById("salt").value=result;document.getElementById("keyGeneratorForm").addEventListener("submit",function(event){event.preventDefault()});return result}function t(key,params={}){const lang=localStorage.getItem("preferredLanguage")||"en";let str=translations[lang]?.[key]||key;for(const[k,v]of Object.entries(params)){str=str.replaceAll(`\${${k}}`,v)}return str}const autoHideTimers={};function showManyPassword(button){const inputId=button.name;const passwordInput=document.getElementById(inputId);if(!passwordInput){showAlert(`No field found for: ${inputId}`,"error");return}const textSpan=button.querySelector(".toggle-text");if(textSpan){textSpan.setAttribute("data-i18n",passwordInput.type==="password"?"hideButton":"showButton");const lang=localStorage.getItem("preferredLanguage")||"en";const key=textSpan.getAttribute("data-i18n");textSpan.innerText=translations[lang]?.[key]||key}if(autoHideTimers[inputId]){clearTimeout(autoHideTimers[inputId]);delete autoHideTimers[inputId]}const isHidden=passwordInput.type==="password";passwordInput.type=isHidden?"text":"password";button.textContent=isHidden?t("hide"):t("show");if(isHidden){autoHideTimers[inputId]=setTimeout(()=>{passwordInput.type="password";button.textContent=t("show");delete autoHideTimers[inputId]},12e4)}}let masterPasswordHideTimeout=null;function showMasterPassword(button){const inputId=button.name;const passwordInput=document.getElementById(inputId);if(!passwordInput){showAlert(`No field found for: ${inputId}`,"error");return}const textSpan=button.querySelector(".toggle-text");const isCurrentlyHidden=passwordInput.type==="password";if(isCurrentlyHidden){passwordInput.type="text";if(textSpan){textSpan.setAttribute("data-i18n","showButton");textSpan.textContent=t("hide")}lucide.createIcons();clearTimeout(masterPasswordHideTimeout);masterPasswordHideTimeout=setTimeout(()=>{passwordInput.type="password";if(textSpan){textSpan.setAttribute("data-i18n","showButton");textSpan.textContent=t("show")}lucide.createIcons()},4e3)}else{clearTimeout(masterPasswordHideTimeout);passwordInput.type="password";if(textSpan){textSpan.setAttribute("data-i18n","showButton");textSpan.textContent=t("show")}lucide.createIcons()}}function clearPasswordCard(){document.getElementById("serviceLabel").innerText="Service Name";generatedUsernames=[];generatedPasswords=[];currentPasswordIndex=0;currentUsernameIndex=0;document.getElementById("passwordPreview").value="";document.getElementById("usernamePreview").value="";document.getElementById("profileSelector").value="structured";document.getElementById("counter").value=0;document.getElementById("passwordIndexLabel").innerText="(1/6)";document.getElementById("usernameIndexLabel").innerText="(1/6)";let generated=document.getElementById("passwordCard");hideSmooth(generated)}if("serviceWorker"in navigator){window.addEventListener("load",function(){navigator.serviceWorker.register("/service-worker.js").then(function(registration){},function(err){console.error("ServiceWorker registration failed: ",err)})})}function segmentKey(key,segmentSize=7){let output="",i=0;while(i+segmentSize<key.length){output+=key.slice(i,i+segmentSize)+"-";i+=segmentSize}return output+key.slice(i)}const fileButton=document.getElementById("fileButton");const fileInput=document.getElementById("fileInput");if(fileButton&&fileInput){fileButton.addEventListener("click",function(){fileInput.click()})}const fileButtonDecrypt=document.getElementById("fileButtonDecrypt");const encryptedFileInput=document.getElementById("encryptedFileInput");if(fileButtonDecrypt&&encryptedFileInput){fileButtonDecrypt.addEventListener("click",function(){encryptedFileInput.click()})}document.addEventListener("DOMContentLoaded",function(){const navLinks=document.querySelectorAll(".nav-link");navLinks.forEach(function(navLink){navLink.addEventListener("click",function(){const navbarCollapse=document.querySelector(".navbar-collapse");if(navbarCollapse.classList.contains("show")){navbarCollapse.classList.remove("show")}})})});function showModal(message){const modal=document.getElementById("alertModalAlert");const modalMessage=document.getElementById("alertModalMessage");modalMessage.textContent=message;modal.style.display="block";const closeButton=document.getElementsByClassName("close")[0];closeButton.onclick=function(){modal.style.display="none"};window.onclick=function(event){if(event.target===modal){modal.style.display="none"}}}function calculateEntropy(password){if(!password)return 0;const charsetSize=getCharsetSize(password);return password.length*Math.log2(charsetSize)}function getCharsetSize(password){const uniqueChars=new Set(password.split(""));return uniqueChars.size}function updatePasswordStrength(password){const strengthBar=document.getElementById("passwordStrengthBar");const strengthText=document.getElementById("passwordStrengthText");const entropyText=document.getElementById("entropyText");let score=0;let hasLower=/[a-z]/.test(password);let hasUpper=/[A-Z]/.test(password);let hasDigit=/\d/.test(password);let hasSpecial=/[!@#$%^&*(),.?":{}|<>]/.test(password);if(!password){strengthText.innerText="";strengthBar.className="";entropyText.innerText="";strengthText.style.color="blue";return}if(password.length>=12){score++}if(password.length>=17){score++}if(hasLower)score++;if(hasUpper)score++;if(hasDigit)score++;if(hasSpecial)score++;const entropy=calculateEntropy(password);let entropyScore=0;let entropyLevel="Low Entropy";if(entropy<30){entropyScore=1;entropyLevel="Low Entropy"}else if(entropy>=30&&entropy<56){entropyScore=2;entropyLevel="Medium Entropy"}else if(entropy>=56&&entropy<85){entropyScore=4;entropyLevel="High Entropy"}else{entropyScore=6;entropyLevel="Extra Strong"}score+=entropyScore;entropyText.innerText=`Entropy: ${entropy.toFixed(2)} (${entropyLevel})`;let strengthLevel="Weak";let strengthColor="red";if(score<=5){strengthLevel="Weak";strengthColor="red"}else if(score<=8){strengthLevel="Medium";strengthColor="orange"}else if(score<=10){strengthLevel="Strong";strengthColor="green"}else{strengthLevel="Extra Strong";strengthColor="#1E003B"}strengthBar.className="";strengthBar.classList.add(strengthLevel.toLowerCase().replace(" ",""));strengthText.innerText=strengthLevel;strengthText.style.color=strengthColor}function generatePasswordAndTrigger(password){document.getElementById("mainPassword").value=password;updatePasswordStrength(password)}function copyPasswordCheck(button){resetTimer();const passwordField=document.getElementById(button.name);const successIcon=document.getElementById("copySuccessIcon-"+button.getAttribute("data-password-id"));navigator.clipboard.writeText(passwordField.value).then(()=>{successIcon.style.display="inline";successIcon.innerHTML='<i data-lucide="copy-check" style="color: green; font-size: 18px;"></i>';lucide.createIcons();document.getElementById("clearClipboardBtn").style.display="flex";setTimeout(()=>{document.getElementById("clearClipboardBtn").style.display="none"},6e5);setTimeout(()=>{successIcon.style.display="none"},2e3)})["catch"](err=>{console.error("Error copying text: ",err)})}document.getElementById("clearClipboardBtn").addEventListener("click",()=>{navigator.clipboard.writeText("").then(()=>{document.getElementById("clearClipboardBtn").style.display="none"})["catch"](err=>{console.warn("Clipboard clear failed:",err);document.getElementById("clearClipboardBtn").style.display="none";showAlert("Unable to clear clipboard. Please copy something else manually to clear sensitive data.")})});function copyPasswordCheckAfterGeneration(){const successIcon=document.getElementById("copySuccessIcon-mainCopy");successIcon.style.display="inline";successIcon.innerHTML='<i data-lucide="copy-check" style="color: green; font-size: 18px;"></i>';lucide.createIcons();setTimeout(()=>{successIcon.style.display="none"},2e3)}let entropyEstimate=null;function conservativeEntropyEstimate(pass){const trimmed=pass.trim();if(!trimmed)return 0;const isLetter=ch=>/\p{L}/u.test(ch);const isDigit=ch=>/\d/.test(ch);const isASCII=ch=>ch.charCodeAt(0)<=127;let hasLower=false,hasUpper=false,hasDigits=false;const symbolSet=new Set;let effectiveLength=0;for(const ch of trimmed){if(/[a-z]/.test(ch))hasLower=true;else if(/[A-Z]/.test(ch))hasUpper=true;else if(isDigit(ch))hasDigits=true;else if(!isLetter(ch)&&isASCII(ch)){symbolSet.add(ch)}effectiveLength++}let charsetSize=0;if(hasLower)charsetSize+=26;if(hasUpper)charsetSize+=26;if(hasDigits)charsetSize+=10;if(symbolSet.size>0)charsetSize+=symbolSet.size;charsetSize=Math.max(1,charsetSize);const charEntropy=effectiveLength*Math.log2(charsetSize);const wordLike=/^[\p{L}\s\-'",.!?]+$/u.test(trimmed);const words=trimmed.split(/[\s\-]+/).filter(w=>/^\p{L}{2,}$/u.test(w));const looksLikeSentence=wordLike&&words.length>=2;if(looksLikeSentence){const wordEntropy=words.length*12;entropyEstimate=Math.min(wordEntropy,charEntropy);return Math.min(wordEntropy,charEntropy)}entropyEstimate=Math.round(charEntropy);return Math.round(charEntropy)}function updateEntropyUI(bits){const bar=document.getElementById("entropyFill");const text=document.getElementById("entropyText");bar.style.width=Math.min(100,bits)+"%";if(bits<60){bar.style.background="#842029"}else if(bits<90){bar.style.background="#d8a305"}else{bar.style.background="#0f5132"}}const encryptPasscode=async passcode=>{const encoder=new TextEncoder;const data=encoder.encode(passcode);const passwordHash=await crypto.subtle.digest("SHA-256",data);const key=await crypto.subtle.importKey("raw",passwordHash,{name:"AES-GCM"},false,["encrypt","decrypt"]);const nonce=crypto.getRandomValues(new Uint8Array(12));const cipherText=await crypto.subtle.encrypt({name:"AES-GCM",iv:nonce},key,passwordHash);const tag=cipherText.slice(-16);const ciphertextWithoutTag=cipherText.slice(0,-16);localStorage.setItem("encryptedPasscode",JSON.stringify({ciphertext:Array.from(new Uint8Array(ciphertextWithoutTag)),nonce:Array.from(nonce),tag:Array.from(new Uint8Array(tag))}));localStorage.setItem("lastAccessTime",Date.now().toString())};const decryptPasscode=async enteredPasscode=>{const storedData=JSON.parse(localStorage.getItem("encryptedPasscode"));if(!storedData)return false;const{ciphertext,nonce,tag}=storedData;const encoder=new TextEncoder;const data=encoder.encode(enteredPasscode);const passwordHash=await crypto.subtle.digest("SHA-256",data);const key=await crypto.subtle.importKey("raw",passwordHash,{name:"AES-GCM"},false,["decrypt"]);const ciphertextUint8=new Uint8Array(ciphertext);const nonceUint8=new Uint8Array(nonce);const tagUint8=new Uint8Array(tag);try{const decryptedData=await crypto.subtle.decrypt({name:"AES-GCM",iv:nonceUint8,additionalData:new Uint8Array,tagLength:128},key,new Uint8Array([...ciphertextUint8,...tagUint8]));return true}catch(e){return false}};let currentIconSet=[];let expectedSequence=[];let selectedRegister=[];let selectedLogin=[];let practiceCount=0;function selectEmojisFromPassphrase(passphrase,count=25){const hashHex=CryptoJS.SHA256(passphrase).toString();const seed=parseInt(hashHex.slice(0,8),16);function mulberry32(a){return function(){let t=a+=1831565813;t=Math.imul(t^t>>>15,t|1);t^=t+Math.imul(t^t>>>7,t|61);return((t^t>>>14)>>>0)/4294967296}}const rng=mulberry32(seed);const indices=Array.from({length:ICON_POOL.length},(_,i)=>i);for(let i=indices.length-1;i>0;i--){const j=Math.floor(rng()*(i+1));[indices[i],indices[j]]=[indices[j],indices[i]]}const selectedIcons=indices.slice(0,count).map(i=>ICON_POOL[i]);renderIconGrid(selectedIcons,"emojiGridRegister",selectedRegister)}function selectIconsFromPassphrase(passphrase,count=25){const hashHex=CryptoJS.SHA256(passphrase).toString(CryptoJS.enc.Hex);const seed=parseInt(hashHex.slice(0,8),16);function mulberry32(a){return function(){let t=a+=1831565813;t=Math.imul(t^t>>>15,t|1);t^=t+Math.imul(t^t>>>7,t|61);return((t^t>>>14)>>>0)/4294967296}}const rng=mulberry32(seed);const indices=Array.from({length:ICON_POOL.length},(_,i)=>i);for(let i=indices.length-1;i>0;i--){const j=Math.floor(rng()*(i+1));[indices[i],indices[j]]=[indices[j],indices[i]]}currentIconSet=indices.slice(0,count).map(i=>ICON_POOL[i]);expectedSequence=currentIconSet.slice(0,6);renderIconGrid(shuffleGridLayout(currentIconSet))}function secureShuffle(array){const arr=[...array];for(let i=arr.length-1;i>0;i--){const rand=new Uint32Array(1);crypto.getRandomValues(rand);const j=rand[0]%(i+1);[arr[i],arr[j]]=[arr[j],arr[i]]}return arr}function selectLoginIconsFromPassphrase(passphrase,count=25){const hashHex=CryptoJS.SHA256(passphrase).toString();const seed=parseInt(hashHex.slice(0,8),16);function mulberry32(a){return function(){let t=a+=1831565813;t=Math.imul(t^t>>>15,t|1);t^=t+Math.imul(t^t>>>7,t|61);return((t^t>>>14)>>>0)/4294967296}}const rng=mulberry32(seed);const indices=Array.from({length:ICON_POOL.length},(_,i)=>i);for(let i=indices.length-1;i>0;i--){const j=Math.floor(rng()*(i+1));[indices[i],indices[j]]=[indices[j],indices[i]]}const selectedIcons=indices.slice(0,count).map(i=>ICON_POOL[i]);shuffledLoginIcons=secureShuffle(selectedIcons);renderIconGrid(shuffledLoginIcons,"emojiGridLogin",selectedLogin)}function shuffleGridLayout(array){const shuffled=array.slice();for(let i=shuffled.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[shuffled[i],shuffled[j]]=[shuffled[j],shuffled[i]]}return shuffled}let shuffledLoginIcons=null;function renderIconGrid(iconNames,gridId,selectedArray){const grid=document.getElementById(gridId);if(!grid)return;grid.innerHTML="";grid.classList.remove("practice-mode");grid.classList.add("icon-grid","grid-mode");iconNames.forEach((name,index)=>{const div=document.createElement("div");div.className="emoji-tile";div.innerHTML=`<i data-lucide="${name}"></i>`;const selectedIndex=selectedArray.findIndex(sel=>sel.name===name);if(selectedIndex!==-1){div.classList.add("selected");const badge=document.createElement("span");badge.className="badge";badge.textContent=selectedIndex+1;div.appendChild(badge)}div.addEventListener("click",()=>{const idx=selectedArray.findIndex(sel=>sel.name===name);if(idx!==-1){selectedArray.splice(idx,1)}else if(selectedArray.length<6){selectedArray.push({index:index,name:name})}renderIconGrid(iconNames,gridId,selectedArray)});grid.appendChild(div)});if(typeof lucide!=="undefined"){lucide.createIcons()}const confirmBtn=document.getElementById("confirmIconSequenceLogin");if(confirmBtn)confirmBtn.disabled=selectedArray.length!==6}function renderPracticeGrid(tileDataArray){const grid=document.getElementById("emojiGridRegister");grid.innerHTML="";grid.classList.remove("grid-mode","practice-mode");grid.classList.add("grid-mode");tileDataArray.forEach((tileData,index)=>{const tile=document.createElement("div");tile.className="emoji-tile";if(!tileData.isPlaceholder&&tileData.name){tile.innerHTML=`<i data-lucide="${tileData.name}"></i>`;const selectedIndex=selectedRegister.findIndex(sel=>sel.name===tileData.name);if(selectedIndex!==-1){tile.classList.add("selected");const badge=document.createElement("span");badge.className="badge";badge.textContent=selectedIndex+1;tile.appendChild(badge)}tile.addEventListener("click",()=>{const selIdx=selectedRegister.findIndex(sel=>sel.name===tileData.name);if(selIdx!==-1){selectedRegister.splice(selIdx,1)}else if(selectedRegister.length<6){selectedRegister.push({index:index,name:tileData.name})}renderPracticeGrid(tileDataArray)})}else{tile.style.visibility="hidden"}grid.appendChild(tile)});if(typeof lucide!=="undefined"){lucide.createIcons()}const confirmBtn=document.getElementById("confirmEmojiSequence");if(confirmBtn)confirmBtn.disabled=selectedRegister.length!==6}function shuffleAndRenderPracticeIcons(){if(!targetEmojiSequence)return;const iconNames=targetEmojiSequence.split("::");const allTiles=[];iconNames.forEach(name=>{allTiles.push({name:name,isPlaceholder:false})});while(allTiles.length<25){allTiles.push({name:null,isPlaceholder:true})}for(let i=allTiles.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[allTiles[i],allTiles[j]]=[allTiles[j],allTiles[i]]}currentPracticeTiles=allTiles;selectedRegister.length=0;renderPracticeGrid(currentPracticeTiles)}function renderLucideGrid(iconSubset,isPractice=false){const grid=document.getElementById("emojiGridRegister");grid.innerHTML="";grid.classList.remove("grid-mode","practice-mode");grid.classList.add("grid-mode");const allTiles=[];iconSubset.forEach(name=>{allTiles.push({name:name,isPlaceholder:false})});while(allTiles.length<25){allTiles.push({name:null,isPlaceholder:true})}if(isPractice){for(let i=allTiles.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[allTiles[i],allTiles[j]]=[allTiles[j],allTiles[i]]}}selected.length=0;allTiles.forEach((tileData,tileIndex)=>{const tile=document.createElement("div");tile.className="emoji-tile";if(!tileData.isPlaceholder&&tileData.name){tile.innerHTML=`<i data-lucide="${tileData.name}"></i>`;const selectedIndex=selected.findIndex(sel=>sel.name===tileData.name);if(selectedIndex!==-1){tile.classList.add("selected");const badge=document.createElement("span");badge.className="badge";badge.textContent=selectedIndex+1;tile.appendChild(badge)}tile.addEventListener("click",()=>{const selIdx=selected.findIndex(sel=>sel.name===tileData.name);if(selIdx!==-1){selected.splice(selIdx,1)}else if(selected.length<6){selected.push({index:tileIndex,name:tileData.name})}renderLucideGrid(iconSubset,isPractice)})}else{tile.style.visibility="hidden"}grid.appendChild(tile)});if(typeof lucide!=="undefined"){lucide.createIcons()}const confirmBtn=document.getElementById("confirmEmojiSequence");if(confirmBtn)confirmBtn.disabled=selected.length!==6}async function getIconSequenceHash(){if(selectedRegister.length!==6){throw new Error("You must select exactly 6 icons.")}const sequence=selectedRegister.map(sel=>sel.name).join("::");const data=(new TextEncoder).encode(sequence);const hashBuffer=await crypto.subtle.digest("SHA-256",data);const hashHex=Array.from(new Uint8Array(hashBuffer)).map(b=>b.toString(16).padStart(2,"0")).join("");return hashHex}async function getLoginIconSequenceHash(){if(selectedLogin.length!==6){throw new Error("Please select exactly 6 icons.")}const sequence=selectedLogin.map(sel=>sel.name).join("::");const data=(new TextEncoder).encode(sequence);const hashBuffer=await crypto.subtle.digest("SHA-256",data);return Array.from(new Uint8Array(hashBuffer)).map(b=>b.toString(16).padStart(2,"0")).join("")}let emojiPracticeAttempts=0;let targetEmojiSequence=null;let currentPracticePass=0;let currentPracticeTiles=null;let iconHash=null;document.getElementById("confirmEmojiSequence").addEventListener("click",async()=>{if(selectedRegister.length!==6){showModalAlert("exactlySix");return}const selectedSequence=selectedRegister.map(sel=>sel.name).join("::");if(emojiPracticeAttempts===0){targetEmojiSequence=selectedSequence;emojiPracticeAttempts++;currentPracticePass=1;shuffleAndRenderPracticeIcons();showModalAlert(t("tryReselect",{currentPracticePass:currentPracticePass}))}else{if(selectedSequence===targetEmojiSequence){currentPracticePass++;if(currentPracticePass>=3){iconHash=await getIconSequenceHash();emojiPracticeAttempts=0;targetEmojiSequence=null;currentPracticePass=0;document.getElementById("confirmEmojiSequence").disabled=true;document.getElementById("submitPassCode").disabled=false;document.getElementById("submitPassCodeRegister").disabled=false;document.getElementById("confirmEmojiSequence").style.visibility="hidden";document.getElementById("submitPassCodeRegister").style.visibility="visible";showModalAlert("sequenceConfirmed")}else{emojiPracticeAttempts++;shuffleAndRenderPracticeIcons();showModalAlert(t("correctSequence",{currentPracticePass:currentPracticePass}))}}else{showModalAlert("wrongSequence");emojiPracticeAttempts=0;targetEmojiSequence=null;currentPracticePass=0;selectedRegister.length=0;let currentPassInput=document.getElementById("passCodeInputRegister");currentPassInput.value="";selectIconsFromPassphrase(currentPassInput);resetAuthState()}}});let iconRegisterTimeout;document.getElementById("passCodeInputRegister").addEventListener("input",e=>{const pass=e.target.value;if(iconRegisterTimeout)clearTimeout(iconRegisterTimeout);iconRegisterTimeout=setTimeout(()=>{if(pass.length>0&&entropyEstimate>89){selectEmojisFromPassphrase(pass)}},400)});let iconLoginTimeout;document.getElementById("passCodeInput").addEventListener("input",e=>{const pass=e.target.value.trim();if(iconLoginTimeout)clearTimeout(iconLoginTimeout);iconLoginTimeout=setTimeout(()=>{if(pass.length>0){selectLoginIconsFromPassphrase(pass)}},400)});async function encryptWithKey(key,data){const iv=crypto.getRandomValues(new Uint8Array(12));const ciphertext=await crypto.subtle.encrypt({name:"AES-GCM",iv:iv},key,data);return{ciphertext:new Uint8Array(ciphertext),iv:iv}}async function decryptWithKey(key,data,iv){return crypto.subtle.decrypt({name:"AES-GCM",iv:iv},key,data)}async function deriveKeyMaterial(passphrase){const encoder=new TextEncoder;const passphraseBuffer=encoder.encode(passphrase);return crypto.subtle.importKey("raw",passphraseBuffer,"PBKDF2",false,["deriveBits","deriveKey"])}async function getHardenedDeterministicSalt(passphrase,context="carrypass-session-v1"){const baseKey=await crypto.subtle.importKey("raw",(new TextEncoder).encode(passphrase),"PBKDF2",false,["deriveBits"]);const pbkdf2Salt=await crypto.subtle.deriveBits({name:"PBKDF2",salt:(new TextEncoder).encode(context),iterations:5e4,hash:"SHA-256"},baseKey,128);const hardenedSalt=new Uint8Array(pbkdf2Salt);const ARGON_SALT_CHARSET="{VkAumXZsJ+]8@olqx)_.F1c&<OPEdnC6N)bMRyY!K,.U4H?g7GIfiDvr*tzLp5039^eThaSBWw2j>/|";const argonSalt=(new TextEncoder).encode(ARGON_SALT_CHARSET);const argonHash=await argon2.hash({pass:passphrase,salt:argonSalt,mem:131072,time:2,parallelism:1,hashLen:16,type:argon2.ArgonType.Argon2id,raw:true});const enhancer=new Uint8Array(argonHash.hash);const finalSalt=new Uint8Array(hardenedSalt.length+enhancer.length);finalSalt.set(hardenedSalt,0);finalSalt.set(enhancer,hardenedSalt.length);return finalSalt}async function deriveHardenedSalt(password,context="carrypass-appMaster-v1"){const enc=new TextEncoder;const hmacKey=await crypto.subtle.importKey("raw",enc.encode(password),{name:"HMAC",hash:"SHA-256"},false,["sign"]);const hardenedSaltBuffer=await crypto.subtle.sign("HMAC",hmacKey,enc.encode(context));return new Uint8Array(hardenedSaltBuffer)}function resetAuthState(){iconHash="";currentIconSet=[];expectedSequence=[];selectedRegister=[];selectedLogin=[];const registerInput=document.getElementById("passCodeInputRegister");const loginInput=document.getElementById("passCodeInput");if(registerInput)registerInput.value="";if(loginInput)loginInput.value="";const overlay=document.getElementById("fingerprint-overlay");const overlayLogin=document.getElementById("fingerprint-overlay-login");if(overlay)overlay.innerHTML="";if(overlayLogin)overlayLogin.innerHTML="";const gridLogin=document.getElementById("emojiGridLogin");const gridRegister=document.getElementById("emojiGridRegister");if(gridLogin)gridLogin.innerHTML="";if(gridRegister)gridRegister.innerHTML="";const hazeSlider=document.getElementById("hazeSlider");const hazeSliderRegister=document.getElementById("hazeSliderRegister");if(hazeSlider)hazeSlider.value=0;if(hazeSliderRegister)hazeSliderRegister.value=0;const hazeOverlay=document.getElementById("gridHazeOverlay");const hazeOverlayRegister=document.getElementById("gridHazeOverlayRegister");if(hazeOverlay)hazeOverlay.classList.add("haze-blackout");if(hazeOverlayRegister)hazeOverlayRegister.classList.add("haze-blackout");const entropyFill=document.getElementById("entropyFill");if(entropyFill)entropyFill.style.width="0%"}const handlePasscodeSubmit=async mode=>{const inputId=mode==="register"?"passCodeInputRegister":"passCodeInput";const passcode=document.getElementById(inputId)?.value.trim();const inputEl=document.getElementById(inputId);if(!passcode||passcode.length<16){showModalAlert("sufficientPassphrase");return}if(!iconHash&&mode==="register"){showModalAlert("iconSequenceMissing");return}if(!iconHash&&mode==="login"){try{iconHash=await getLoginIconSequenceHash()}catch{showModalAlert("selectSix");return}}const combinedPass=passcode+"::"+iconHash;if(mode==="register"){const estimatedEntropy=conservativeEntropyEstimate(passcode);if(estimatedEntropy<90){showModalAlert(t("weakPassphrase",{estimatedEntropy:estimatedEntropy}));return}}const spinner1=document.getElementById("spinner1");spinner1.style.visibility="visible";const spinner2=document.getElementById("spinner2");spinner2.style.visibility="visible";const masterKeyMaterial=await deriveKeyMaterial(combinedPass);const sessionSalt=await getHardenedDeterministicSalt(combinedPass);sessionKey=await deriveAESKeyLogin(masterKeyMaterial,sessionSalt);await decryptStoredTOTPSecret();await decryptStoredLabel();const baseKey=await crypto.subtle.importKey("raw",(new TextEncoder).encode(combinedPass),"PBKDF2",false,["deriveBits"]);const normalizedBits=await crypto.subtle.deriveBits({name:"PBKDF2",salt:(new TextEncoder).encode("carrypass-appMaster-v1"),iterations:5e4,hash:"SHA-256"},baseKey,256);const normalizedKey=new Uint8Array(normalizedBits);const ARGON_CHARSET="aZ9@Vx#r7]O{!Dk|Nq$H^J2W(f4*)ML+S&GCpT_EYIB3XU[0}5lt1n6?AvdRcmoKszFjihb8ugP,.<>/";const argonSalt=(new TextEncoder).encode(ARGON_CHARSET);const argonHash=await argon2.hash({pass:normalizedKey,salt:argonSalt,mem:131072,time:3,parallelism:1,hashLen:32,type:argon2.ArgonType.Argon2id,raw:true});const hardenedBytes=new Uint8Array(argonHash.hash);appMaster=Array.from(hardenedBytes).map(b=>b.toString(16).padStart(2,"0")).join("");if(mode==="register"){const storedCheckSalt=crypto.getRandomValues(new Uint8Array(16));const checkKey=await deriveAESKeyLogin(masterKeyMaterial,storedCheckSalt);const checkValue=(new TextEncoder).encode("CarryPassCheck");const{ciphertext,iv}=await encryptWithKey(checkKey,checkValue);localStorage.setItem("encryptedPasscode",JSON.stringify({iv:Array.from(iv),salt:Array.from(storedCheckSalt),ciphertext:Array.from(ciphertext)}));localStorage.setItem("lastAccessTime",Date.now().toString());inputEl.value="";updateTrustedDeviceIndicator();showScreen("appView");startLockCountdown();resetAuthState()}else{const stored=JSON.parse(localStorage.getItem("encryptedPasscode"));if(!stored)return showModalAlert("registrationMissing");const salt=new Uint8Array(stored.salt);const iv=new Uint8Array(stored.iv);const encrypted=new Uint8Array(stored.ciphertext);const checkKey=await deriveAESKeyLogin(masterKeyMaterial,salt);try{const plain=await decryptWithKey(checkKey,encrypted,iv);if((new TextDecoder).decode(plain)==="CarryPassCheck"){localStorage.setItem("lastAccessTime",Date.now().toString());inputEl.value="";updateTrustedDeviceIndicator();updateIdentityIndicator();try{await loadServiceSettingsBlobs(sessionKey);buildSavedServicesList()}catch(err){console.error("Failed to load saved services:",err);showModalAlert("notLoadServices")}await loadCustomProfilesIntoSelector();showScreen("appView");startLockCountdown();resetAuthState()}else{showModalAlert("invalidPersonalKey");resetAuthState()}}catch{showModalAlert("invalidPersonalKey");resetAuthState()}}spinner1.style.visibility="hidden";spinner2.style.visibility="hidden"};document.getElementById("submitPassCode").addEventListener("click",()=>handlePasscodeSubmit("login"));document.getElementById("submitPassCodeRegister").addEventListener("click",()=>handlePasscodeSubmit("register"));async function deriveAESKeyLogin(material,salt){return crypto.subtle.deriveKey({name:"PBKDF2",salt:salt,iterations:5e4,hash:"SHA-256"},material,{name:"AES-GCM",length:256},false,["encrypt","decrypt"])}let sessionKey=null;let appMaster=null;let encryptedAppMaster=null;let useAppIdentity=true;let screenLockActive=false;let cachedServiceList=[];let cachedServiceMap={};let settingsDirty=false;const AUTO_SAVE_INTERVAL=5*60*1e3;let pinRetryCount=0;const MAX_PIN_RETRIES=3;let lastActiveScreen=null;const DEFAULT_PIN="000000";const PIN_SALT="carrypass-screenlock-v1";async function deriveScreenKey(pin){const salt=(new TextEncoder).encode(PIN_SALT);const pinKey=await crypto.subtle.importKey("raw",(new TextEncoder).encode(pin),"PBKDF2",false,["deriveKey"]);return await crypto.subtle.deriveKey({name:"PBKDF2",salt:salt,iterations:1e5,hash:"SHA-256"},pinKey,{name:"AES-GCM",length:256},false,["encrypt","decrypt"])}let lastInteractionTime=Date.now();let lockTimer=null;const LOCK_AFTER_MINUTES=10;function refreshLockTimer(){lastInteractionTime=Date.now()}function startLockCountdown(){if(lockTimer)clearInterval(lockTimer);lockTimer=setInterval(()=>{if(Date.now()-lastInteractionTime>LOCK_AFTER_MINUTES*60*1e3){triggerAutoLock()}},6e4)}function triggerAutoLock(){if(identity&&appMaster){loadDecryptedPIN().then(pin=>{lockAppWithPIN(pin||DEFAULT_PIN)})}}const PIN_HASH_KEY="carrypass_screenPinHash";async function saveCustomPIN(pin){const salt=(new TextEncoder).encode("carrypass-pin-hash");const key=await crypto.subtle.importKey("raw",(new TextEncoder).encode(pin),"PBKDF2",false,["deriveBits"]);const hashBuffer=await crypto.subtle.deriveBits({name:"PBKDF2",salt:salt,iterations:1e5,hash:"SHA-256"},key,256);const hashArray=Array.from(new Uint8Array(hashBuffer));localStorage.setItem("carrypass_screenPinHash",JSON.stringify(hashArray))}function getStoredPINHash(){const raw=localStorage.getItem("carrypass_screenPinHash");return raw?JSON.parse(raw):null}function getStoredPIN(){return localStorage.getItem("carrypass_screenPinHash")}document.getElementById("savePinBtn").addEventListener("click",async()=>{const pin=document.getElementById("setPinInput").value.trim();if(!pin||pin.length<4){showModalAlert("pinMin");return}await saveEncryptedPIN(pin);showModalAlert("pinSaved");document.getElementById("pinSettingsModal").classList.add("hidden")});let identity=true;function updateIdentityIndicator(){const indicator=document.getElementById("identityIndicator");const label=indicator.querySelector(".identity-label");const icon=document.getElementById("identityToggleIcon");const identityFields=document.getElementById("identityModeFields");const manualFields=document.getElementById("manualModeFields");if(identity){indicator.classList.remove("off");label.textContent=t("identityOn");indicator.title="Identity mode enabled";icon.setAttribute("data-lucide","orbit");if(identityFields)identityFields.style.display="block";if(manualFields)manualFields.style.display="none"}else{indicator.classList.add("off");label.textContent=t("identityOff");indicator.title="Identity mode disabled";icon.setAttribute("data-lucide","circuit-board");if(identityFields)identityFields.style.display="none";if(manualFields)manualFields.style.display="block"}if(typeof lucide!=="undefined")lucide.createIcons()}function toggleIdentityMode(){identity=!identity;updateIdentityIndicator()}document.addEventListener("DOMContentLoaded",()=>{document.getElementById("toggleIdentityBtn").addEventListener("click",toggleIdentityMode);updateIdentityIndicator()});function updatePasswordIndexLabel(index){const label=document.getElementById("passwordIndexLabel");if(label)label.textContent=`(${index+1}/6)`}function updateUsernameIndexLabel(index){const label=document.getElementById("usernameIndexLabel");if(label)label.textContent=`(${index+1}/6)`}function displayPasswordFromIndex(index){const passwordOutput=document.getElementById("passwordPreview");if(passwordOutput&&generatedPasswords[index]){passwordOutput.value=generatedPasswords[index]}}function displayUsernameFromIndex(index){const usernameOutput=document.getElementById("usernamePreview");if(usernameOutput&&generatedUsernames[index]){usernameOutput.value=generatedUsernames[index]}}async function encryptSettingsData(obj,key){const encoded=(new TextEncoder).encode(JSON.stringify(obj));const nonce=crypto.getRandomValues(new Uint8Array(12));const ciphertext=await crypto.subtle.encrypt({name:"AES-GCM",iv:nonce},key,encoded);return{nonce:Array.from(nonce),ciphertext:Array.from(new Uint8Array(ciphertext))}}async function decryptSettingsData(stored,key){const nonce=new Uint8Array(stored.nonce);const ciphertext=new Uint8Array(stored.ciphertext);const decrypted=await crypto.subtle.decrypt({name:"AES-GCM",iv:nonce},key,ciphertext);return JSON.parse((new TextDecoder).decode(decrypted))}async function loadServiceSettingsBlobs(key){sessionKey=key;const listRaw=localStorage.getItem("serviceListEncrypted");const mapRaw=localStorage.getItem("serviceMapEncrypted");cachedServiceList=listRaw?await decryptSettingsData(JSON.parse(listRaw),sessionKey):[];cachedServiceMap=mapRaw?await decryptSettingsData(JSON.parse(mapRaw),sessionKey):{}}async function saveServiceSettingsBlobs(){if(!settingsDirty||!sessionKey)return;settingsDirty=false;const listEncrypted=await encryptSettingsData(cachedServiceList,sessionKey);const mapEncrypted=await encryptSettingsData(cachedServiceMap,sessionKey);localStorage.setItem("serviceListEncrypted",JSON.stringify(listEncrypted));localStorage.setItem("serviceMapEncrypted",JSON.stringify(mapEncrypted))}function getServiceSettings(serviceName){return cachedServiceMap[serviceName]||null}function updateServiceSettings(serviceName,updates){if(!cachedServiceList.includes(serviceName)){cachedServiceList.push(serviceName)}cachedServiceMap[serviceName]={...cachedServiceMap[serviceName]||{},...updates};settingsDirty=true}let debounceTimers={};function debounceUpdate(serviceName,field,value,delay=1e3){const key=`${serviceName}_${field}`;clearTimeout(debounceTimers[key]);debounceTimers[key]=setTimeout(()=>{updateServiceSettings(serviceName,{[field]:value})},delay)}window.addEventListener("beforeunload",async()=>{await saveServiceSettingsBlobs()});setInterval(()=>{saveServiceSettingsBlobs()},AUTO_SAVE_INTERVAL);async function deriveSessionKey(passcode){const encoder=new TextEncoder;const hash=await crypto.subtle.digest("SHA-256",encoder.encode(passcode));const key=await crypto.subtle.importKey("raw",hash,{name:"AES-GCM"},false,["encrypt","decrypt"]);sessionKey=key}let sessionTOTPSecret=null;async function decryptStoredTOTPSecret(){const encryptedQR=localStorage.getItem("encryptedQR");if(!encryptedQR){sessionTOTPSecret=null;return}try{const{ciphertext,nonce,tag}=JSON.parse(encryptedQR);const key=sessionKey;const decrypted=await crypto.subtle.decrypt({name:"AES-GCM",iv:new Uint8Array(nonce),tagLength:128},key,new Uint8Array([...ciphertext,...tag]));sessionTOTPSecret=(new TextDecoder).decode(decrypted)}catch(err){sessionTOTPSecret=null;console.warn("⚠️ Failed to decrypt TOTP secret:",err)}}async function saveEncryptedPIN(pin){resetTimer();if(!sessionKey){console.error("Session key is not available");return}const iv=crypto.getRandomValues(new Uint8Array(12));const encodedPIN=(new TextEncoder).encode(pin);const encrypted=await crypto.subtle.encrypt({name:"AES-GCM",iv:iv},sessionKey,encodedPIN);const payload={iv:Array.from(iv),cipher:Array.from(new Uint8Array(encrypted))};localStorage.setItem("carrypass_encryptedPIN",JSON.stringify(payload))}async function loadDecryptedPIN(){if(!sessionKey)return DEFAULT_PIN;const raw=localStorage.getItem("carrypass_encryptedPIN");if(!raw)return DEFAULT_PIN;try{const{iv,cipher}=JSON.parse(raw);const decrypted=await crypto.subtle.decrypt({name:"AES-GCM",iv:new Uint8Array(iv)},sessionKey,new Uint8Array(cipher));return(new TextDecoder).decode(decrypted)}catch(err){console.warn("Falling back to default PIN due to decryption error:",err);return DEFAULT_PIN}}function getCurrentScreenId(){const screens=document.querySelectorAll(".screen");for(const screen of screens){const style=window.getComputedStyle(screen);if(style.display!=="none"){return screen.id}}return"appView"}async function lockAppWithPIN(pin){const salt=(new TextEncoder).encode("carrypass-screenlock-v1");const pinKey=await crypto.subtle.importKey("raw",(new TextEncoder).encode(pin),"PBKDF2",false,["deriveKey"]);const screenKey=await crypto.subtle.deriveKey({name:"PBKDF2",salt:salt,iterations:1e5,hash:"SHA-256"},pinKey,{name:"AES-GCM",length:256},false,["encrypt","decrypt"]);const iv=crypto.getRandomValues(new Uint8Array(12));const encoded=(new TextEncoder).encode(appMaster);const encrypted=await crypto.subtle.encrypt({name:"AES-GCM",iv:iv},screenKey,encoded);encryptedAppMaster={iv:Array.from(iv),cipher:Array.from(new Uint8Array(encrypted))};appMaster=null;screenLockActive=true;pinRetryCount=0;lastActiveScreen=getCurrentScreenId();showScreen("screenLockView")}async function unlockWithPIN(){const pin=document.getElementById("pinInput")?.value.trim();const pinInput=document.getElementById("pinInput");if(!pin||!encryptedAppMaster)return;const salt=(new TextEncoder).encode("carrypass-screenlock-v1");const pinKey=await crypto.subtle.importKey("raw",(new TextEncoder).encode(pin),"PBKDF2",false,["deriveKey"]);const screenKey=await crypto.subtle.deriveKey({name:"PBKDF2",salt:salt,iterations:1e5,hash:"SHA-256"},pinKey,{name:"AES-GCM",length:256},false,["decrypt"]);try{const decrypted=await crypto.subtle.decrypt({name:"AES-GCM",iv:new Uint8Array(encryptedAppMaster.iv)},screenKey,new Uint8Array(encryptedAppMaster.cipher));appMaster=(new TextDecoder).decode(decrypted);encryptedAppMaster=null;screenLockActive=false;pinRetryCount=0;pinInput.value="";if(lastActiveScreen){showScreen(lastActiveScreen);lastActiveScreen=null}else{showScreen("appView")}}catch{pinRetryCount++;pinInput.value="";if(pinRetryCount>=MAX_PIN_RETRIES){encryptedAppMaster=null;showModalAlert("pinIncorrectRelogin");showScreen("loginView")}else{const remaining=MAX_PIN_RETRIES-pinRetryCount;showModalAlert(t("incorrectPinAttempts",{remaining:remaining}))}}}let sessionMemberId=null;async function decryptStoredLabel(){const encryptedLabel=localStorage.getItem("encryptedLabel");if(!encryptedLabel){sessionMemberId=null;return}try{const{ciphertext,nonce,tag}=JSON.parse(encryptedLabel);const key=sessionKey;const decrypted=await crypto.subtle.decrypt({name:"AES-GCM",iv:new Uint8Array(nonce),tagLength:128},key,new Uint8Array([...ciphertext,...tag]));sessionMemberId=(new TextDecoder).decode(decrypted)}catch(e){sessionMemberId=null;console.warn("⚠️ Failed to decrypt label:",e)}}function setCharTypesValues(){document.getElementById("length").value="28";document.getElementById("iterationCount").value="100";document.getElementById("toggle1").checked=true;document.getElementById("toggle2").checked=true;document.getElementById("toggle3").checked=true;document.getElementById("toggle4").checked=false;document.getElementById("toggle5").checked=true}const saveSettings=async()=>{const rawAddress=document.getElementById("webAddress").value.trim();const normalized=normalizeServiceName(rawAddress);if(!normalized)return;const settings={webAddress:rawAddress,length:document.getElementById("length").value,iterationCount:document.getElementById("iterationCount").value,uppercase:document.getElementById("toggle1").checked,lowercase:document.getElementById("toggle2").checked,numbers:document.getElementById("toggle3").checked,symbols:document.getElementById("toggle4").checked,separator:document.getElementById("toggle5").checked};await encryptSettings(normalized,settings);buildSavedServicesList()};const encryptSettings=async(serviceName,settings)=>{if(!sessionKey){console.error("No session key.");return}const encoder=new TextEncoder;const data=encoder.encode(JSON.stringify(settings));const nonce=crypto.getRandomValues(new Uint8Array(12));const encrypted=await crypto.subtle.encrypt({name:"AES-GCM",iv:nonce},sessionKey,data);const tag=encrypted.slice(-16);const ciphertextWithoutTag=encrypted.slice(0,-16);localStorage.setItem(`encryptedSettings_${serviceName}`,JSON.stringify({ciphertext:Array.from(new Uint8Array(ciphertextWithoutTag)),nonce:Array.from(nonce),tag:Array.from(new Uint8Array(tag))}))};const decryptSettingsFromMemory=async serviceName=>{if(!sessionKey){console.error("No session key available. Cannot decrypt settings.");return null}const storedRaw=localStorage.getItem(`encryptedSettings_${serviceName}`);if(!storedRaw){return null}const stored=JSON.parse(storedRaw);const ciphertext=new Uint8Array(stored.ciphertext.concat(stored.tag));const nonce=new Uint8Array(stored.nonce);try{const decrypted=await crypto.subtle.decrypt({name:"AES-GCM",iv:nonce},sessionKey,ciphertext);const decoded=(new TextDecoder).decode(decrypted);return JSON.parse(decoded)}catch(e){return null}};let lastLoadedService="";document.getElementById("webAddress").addEventListener("input",()=>{const currentInput=normalizeServiceName(document.getElementById("webAddress").value.trim());if(lastLoadedService&&currentInput!==lastLoadedService){document.getElementById("length").value="28";document.getElementById("iterationCount").value="100";document.getElementById("toggle1").checked=true;document.getElementById("toggle2").checked=true;document.getElementById("toggle3").checked=true;document.getElementById("toggle4").checked=false;document.getElementById("toggle5").checked=true;updateArgonTierLabel()}});document.addEventListener("DOMContentLoaded",()=>{document.getElementById("toggle1Admin").checked=true;document.getElementById("toggle2Admin").checked=true;document.getElementById("toggle3Admin").checked=true;document.getElementById("toggle4Admin").checked=true;document.getElementById("toggle5Admin").checked=false;document.getElementById("saltAdmin").value="";document.getElementById("lengthAdmin").value=43;document.getElementById("iterationCountAdmin").value=5e4});const webAddressInput=document.getElementById("webAddress");const dot=document.getElementById("dot");let debounceTimer;function runServiceMatchCheck(){const input=webAddressInput.value.trim();if(!input){dot.style.backgroundColor="";return}const settings=cachedServiceMap[input];if(!settings){dot.style.backgroundColor="red";const profileSelector=document.getElementById("profileSelector");if(profileSelector)profileSelector.value="structured";const variantInput=document.getElementById("counter");if(variantInput)variantInput.value=0;sessionStorage.removeItem("carrypass_indexPassword");sessionStorage.removeItem("carrypass_indexUsername");lastLoadedService="";return}dot.style.backgroundColor="green";const profileSelector=document.getElementById("profileSelector");if(profileSelector&&settings.passwordType){profileSelector.value=settings.passwordType.toLowerCase()}const variantInput=document.getElementById("counter");if(variantInput&&typeof settings.variant==="number"){variantInput.value=settings.variant}if(typeof settings.passwordIndex==="number"){sessionStorage.setItem("carrypass_indexPassword",settings.passwordIndex)}if(typeof settings.usernameIndex==="number"){sessionStorage.setItem("carrypass_indexUsername",settings.usernameIndex)}lastLoadedService=input}webAddressInput.addEventListener("input",()=>{clearTimeout(debounceTimer);debounceTimer=setTimeout(runServiceMatchCheck,300)});webAddressInput.addEventListener("blur",()=>{clearTimeout(debounceTimer);runServiceMatchCheck()});function handleSessionTimeout(){const storedPasscode=localStorage.getItem("encryptedPasscode");if(storedPasscode){showScreen("loginView")}else{showScreen("registrationView")}}const checkSessionTimeout=()=>{const lastAccessTime=parseInt(localStorage.getItem("lastAccessTime"))||0;const currentTime=Date.now();if(currentTime-lastAccessTime>60*60*1e3){const storedPasscode=localStorage.getItem("encryptedPasscode");if(sessionKey)sessionKey=null;if(appMaster)appMaster=null;if(encryptedAppMaster instanceof Uint8Array)encryptedAppMaster.fill(0);encryptedAppMaster=null;sessionTOTPSecret=null;sessionMemberId=null;vault={};document.getElementById("serviceLabel").innerText="Service Name";generatedUsernames=[];generatedPasswords=[];currentPasswordIndex=0;currentUsernameIndex=0;document.getElementById("passwordPreview").value="";document.getElementById("usernamePreview").value="";document.getElementById("profileSelector").value="structured";document.getElementById("counter").value=0;document.getElementById("passwordIndexLabel").innerText="(1/6)";document.getElementById("usernameIndexLabel").innerText="(1/6)";if(storedPasscode){showScreen("loginView")}else{showScreen("registrationView")}}};const resetSessionTimer=()=>{localStorage.setItem("lastAccessTime",Date.now().toString());updateCountdownDisplay()};setInterval(checkSessionTimeout,30*1e3);if(!localStorage.getItem("encryptedPasscode")){showScreen("registrationView")}document.addEventListener("click",e=>{const link=e.target.closest("[data-section]");if(!link)return;e.preventDefault();const screenId=link.getAttribute("data-section");showScreen(screenId)});function clearSensitiveInputs(){const fieldIds=["memberId","memberPassword","memberTOTPInput","adminAccessInput","adminPasswordInput","adminTOTPInput","adminPasswordEditorInput","adminEditorTOTPInput","passCodeInput","passCodeInputRegister"];fieldIds.forEach(id=>{const el=document.getElementById(id);if(el)el.value=""});const fileInputs=document.querySelectorAll('input[type="file"]');fileInputs.forEach(input=>{input.value=""})}function showScreen(screenId){document.querySelectorAll(".screen").forEach(screen=>{if(screen.id===screenId){screen.style.display="block";screen.classList.remove("active");void screen.offsetWidth;setTimeout(()=>{screen.classList.add("active")},10);if(screenId!=="screenLockView"){lastActiveScreen=screenId}}else{screen.classList.remove("active");setTimeout(()=>{screen.style.display="none"},400)}});if((screenId==="loginView"||screenId==="registrationView")&&document.querySelector('.modal.active, .modal.visible, .modal.show, .modal[open], .modal[style*="block"]')){closeAllModals()}if(screenId==="loginView"||screenId==="registrationView"){if(typeof vault==="object"&&vault!==null){for(const key in vault){if(Object.hasOwn(vault,key)){delete vault[key]}}}}}const resetTimer=()=>{const lastAccessTime=parseInt(localStorage.getItem("lastAccessTime"))||0;const currentTime=Date.now();if(currentTime-lastAccessTime>60*60*1e3){const storedPasscode=localStorage.getItem("encryptedPasscode");if(sessionKey)sessionKey=null;if(appMaster)appMaster=null;if(encryptedAppMaster instanceof Uint8Array)encryptedAppMaster.fill(0);encryptedAppMaster=null;sessionTOTPSecret=null;sessionMemberId=null;vault={};document.getElementById("serviceLabel").innerText="Service Name";generatedUsernames=[];generatedPasswords=[];currentPasswordIndex=0;currentUsernameIndex=0;document.getElementById("passwordPreview").value="";document.getElementById("usernamePreview").value="";document.getElementById("profileSelector").value="structured";document.getElementById("counter").value=0;document.getElementById("passwordIndexLabel").innerText="(1/6)";document.getElementById("usernameIndexLabel").innerText="(1/6)";if(storedPasscode){showScreen("loginView")}else{showScreen("registrationView")}}else{localStorage.setItem("lastAccessTime",currentTime.toString());updateCountdownDisplay()}};function closeAllModals(){if(window.openModals){window.openModals.forEach(closeFn=>{try{if(typeof closeFn==="function")closeFn()}catch(e){console.warn("Error closing tracked modal:",e)}});window.openModals=[]}document.querySelectorAll(".modal").forEach(modal=>{modal.classList.remove("active","visible","show");if(!modal.classList.contains("hidden")){modal.classList.add("hidden")}modal.style.display="none";if(typeof modal.close==="function"&&modal.hasAttribute("open")){try{modal.close()}catch(e){}}});document.querySelectorAll(".modal-backdrop").forEach(b=>b.remove())}function switchOff(skipShowScreen=false){document.querySelectorAll("input, textarea").forEach(el=>{if(el.type!=="checkbox"&&el.type!=="radio"){el.value=""}else{el.checked=false}});["mainPassword","username","supplementaryone","supplementarytwo","supplementarythree","supplementaryfour","supplementaryfive"].forEach(id=>{const el=document.getElementById(id);if(el)el.value=""});if(sessionKey)sessionKey=null;if(appMaster)appMaster=null;if(encryptedAppMaster instanceof Uint8Array)encryptedAppMaster.fill(0);encryptedAppMaster=null;sessionTOTPSecret=null;sessionMemberId=null;document.getElementById("serviceLabel").innerText="Service Name";generatedUsernames=[];generatedPasswords=[];currentPasswordIndex=0;currentUsernameIndex=0;document.getElementById("passwordPreview").value="";document.getElementById("usernamePreview").value="";document.getElementById("profileSelector").value="structured";document.getElementById("counter").value=0;document.getElementById("passwordIndexLabel").innerText="(1/6)";document.getElementById("usernameIndexLabel").innerText="(1/6)";document.querySelectorAll(".modal, .modal-edit, .modal-confirm, .carrypass-modal").forEach(modal=>{modal.classList.add("hidden");modal.classList.remove("active");modal.style.display="none"});const backdrop=document.getElementById("modalBackdrop");if(backdrop)backdrop.classList.add("hidden");document.querySelectorAll(".modal-error").forEach(errBox=>{errBox.textContent="";errBox.classList.add("hidden");errBox.classList.remove("active")});vault={};adminConfig=null;securelyEraseMemory();const fieldsToClear=["adminAccessInput","adminPasswordInput","adminTOTPInput","adminPasswordEditorInput","adminEditorTOTPInput"];fieldsToClear.forEach(id=>{const el=document.getElementById(id);if(el)el.value=""});const elementsToClear=["teamsList","membersList","credentialsList","assignmentsList","vaultExportFeedback","vaultExpirationDisplay"];elementsToClear.forEach(id=>{const el=document.getElementById(id);if(el)el.innerHTML=""});const searchInputs=["teamSearchInput","memberSearchInput","credentialSearchInput","credentialSearchByTeam","assignmentSearchInput","assignmentSearchByTeam"];searchInputs.forEach(id=>{const el=document.getElementById(id);if(el)el.value=""});const qrOutput=document.getElementById("qrOutput");if(qrOutput)qrOutput.innerHTML="";const expirySortBtn=document.getElementById("toggleExpirySort");if(expirySortBtn)expirySortBtn.setAttribute("data-active","false");if(!skipShowScreen){const storedPasscode=localStorage.getItem("encryptedPasscode");showScreen(storedPasscode?"loginView":"registrationView")}}function deleteQrCode(){const qrOutput=document.getElementById("qrOutput");const qrPlace=document.getElementById("hiddenQr");if(qrOutput)qrOutput.innerHTML="";if(qrOutput)qrPlace.style.display="none"}document.addEventListener("DOMContentLoaded",()=>{const resetButton=document.getElementById("resetAppButton");if(resetButton){resetButton.addEventListener("click",async()=>{setTimeout(async()=>{const message=t("resetApplicationConfirm");const confirmed=await confirmModal(message);if(confirmed){resetCompleteApp()}},100)})}});function showConfirmModal(message,onConfirm){const modal=document.getElementById("confirmationModal");const msg=document.getElementById("confirmationMessage");const yesBtn=document.getElementById("confirmYesBtn");const noBtn=document.getElementById("confirmNoBtn");msg.textContent=message;modal.classList.remove("hidden");const close=()=>{modal.classList.add("hidden");yesBtn.removeEventListener("click",confirmHandler);noBtn.removeEventListener("click",cancelHandler)};const confirmHandler=()=>{close();if(typeof onConfirm==="function")onConfirm()};const cancelHandler=()=>close();yesBtn.addEventListener("click",confirmHandler);noBtn.addEventListener("click",cancelHandler)}function resetCompleteApp(){localStorage.clear();if(typeof sessionKey!=="undefined")sessionKey="";if(typeof appMaster!=="undefined")appMaster="";if(typeof encryptedAppMaster!=="undefined")encryptedAppMaster="";document.querySelectorAll("input, textarea").forEach(el=>{if(el.type!=="checkbox"&&el.type!=="radio"){el.value=""}else{el.checked=false}});document.querySelectorAll(".modal, .modal-edit, .modal-confirm, .carrypass-modal").forEach(modal=>{modal.classList.add("hidden");modal.classList.remove("active");modal.style.display="none"});const backdrop=document.getElementById("modalBackdrop");if(backdrop)backdrop.classList.add("hidden");document.querySelectorAll(".modal-error").forEach(errBox=>{errBox.textContent="";errBox.classList.add("hidden");errBox.classList.remove("active")});["mainPassword","username","supplementaryone","supplementarytwo","supplementarythree","supplementaryfour","supplementaryfive"].forEach(id=>{const el=document.getElementById(id);if(el)el.value=""});const savedList=document.getElementById("savedServicesList");if(savedList)savedList.innerHTML="";localStorage.clear();sessionStorage.clear();location.reload()}document.getElementById("appViewLogout").addEventListener("click",switchOff);document.getElementById("adminEditViewLogout").addEventListener("click",switchOff);document.getElementById("memberPanelLogout").addEventListener("click",switchOff);document.getElementById("memberVaultLogout").addEventListener("click",switchOff);document.getElementById("adminSetupLogout").addEventListener("click",switchOff);document.getElementById("adminEditorLoginLogout").addEventListener("click",switchOff);document.getElementById("adminBootstrapLogout").addEventListener("click",switchOff);window.addEventListener("load",checkSessionTimeout);window.addEventListener("popstate",checkSessionTimeout);window.addEventListener("hashchange",checkSessionTimeout);document.addEventListener("DOMContentLoaded",function(){lucide.createIcons()});document.addEventListener("DOMContentLoaded",()=>{const hasPasscode=!!localStorage.getItem("encryptedPasscode");if(hasPasscode){showScreen("loginView")}else{showScreen("registrationView")}});function sanitizeServiceNameStringForBase64(input){let normalized=input.normalize("NFD").replace(/[\u0300-\u036f]/g,"");const manualMap={"ß":"ss","ø":"o","Ø":"O","æ":"ae","Æ":"AE","œ":"oe","Œ":"OE","ð":"d","Ð":"D","þ":"th","Þ":"Th"};normalized=normalized.replace(/[^\x00-\x7F]/g,char=>manualMap[char]||"");return normalized.replace(/[^a-zA-Z0-9]/g,"").trim().toLowerCase()}function sanitizeStringForBase64(input){let normalized=input.normalize("NFD").replace(/[\u0300-\u036f]/g,"");const manualMap={"ß":"ss","ø":"o","Ø":"O","æ":"ae","Æ":"AE","œ":"oe","Œ":"OE","ð":"d","Ð":"D","þ":"th","Þ":"Th"};normalized=normalized.replace(/[^\x00-\x7F]/g,char=>manualMap[char]||"");return normalized.replace(/[^a-zA-Z0-9\s?!);/.,:'"@#$%^&*_\-+=<>[\]{}|\\~`]/g,"")}const counterInput=document.getElementById("counter");counterInput.addEventListener("input",e=>{e.target.value=e.target.value.replace(/\D/g,"")});counterInput.addEventListener("keydown",e=>{const allowedKeys=["Backspace","ArrowLeft","ArrowRight","Delete","Tab"];if(!/\d/.test(e.key)&&!allowedKeys.includes(e.key)){e.preventDefault()}});function showSmooth(element){if(!element)return;resetTimer();element.classList.add("visible")}function hideSmooth(element){if(!element)return;resetTimer();element.classList.remove("visible")}function toggleSliderState(sliderId,buttonId){const slider=document.getElementById(sliderId);const ball=document.getElementById(buttonId);var settingsState=document.getElementById("settings");resetTimer();if(slider.classList.contains("active")){slider.classList.remove("active");ball.classList.remove("active");if(buttonId=="btn1"){hideSmooth(settingsState)}}else{slider.classList.add("active");ball.classList.add("active");if(buttonId=="btn1"){showSmooth(settingsState)}}}function activateToggle(){const toggle1=document.getElementById("toggle1");toggle1.checked=true;const toggle2=document.getElementById("toggle2");toggle2.checked=true;const toggle3=document.getElementById("toggle3");toggle3.checked=true;const toggle4=document.getElementById("toggle4");toggle4.checked=false;const toggle5=document.getElementById("toggle5");toggle5.checked=true;const event=new Event("change");toggle1.dispatchEvent(event);toggle2.dispatchEvent(event);toggle3.dispatchEvent(event);toggle4.dispatchEvent(event);toggle5.dispatchEvent(event);resetTimer()}document.addEventListener("DOMContentLoaded",event=>{activateToggle()});document.getElementById("iterationCount").addEventListener("input",function(){const value=this.value;if(value.length>5){this.value=value.slice(0,5)}});document.getElementById("length").addEventListener("input",function(){const value=this.value;if(value.length>3){this.value=value.slice(0,3)}});document.getElementById("iterationCount").addEventListener("focus",function(){this.value=""});document.getElementById("iterationCount").addEventListener("blur",function(){if(this.value===""){this.value="100"}});document.getElementById("length").addEventListener("focus",function(){this.value=""});document.getElementById("length").addEventListener("blur",function(){if(this.value===""){this.value="28"}});document.getElementById("iterationCount").addEventListener("input",function(){if(this.value==="0"){this.value=""}else if(this.value.startsWith("0")&&this.value.length>1){this.value=this.value.slice(1)}});document.getElementById("length").addEventListener("input",function(){if(this.value==="0"){this.value=""}else if(this.value.startsWith("0")&&this.value.length>1){this.value=this.value.slice(1)}});function normalizeServiceName(service){if(!service)return"";return service.trim().toLowerCase().replace(/^https?:\/\//,"").replace(/^www\./,"")}function generateSafeId(name){return name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"")}function collectCustomProfileFromAdminUI(){return{length:parseInt(document.getElementById("lengthCustomSettings").value,10),iterationCount:parseInt(document.getElementById("iterationCountCustomSettings").value,10),uppercase:document.getElementById("toggle1CustomSettings").checked,lowercase:document.getElementById("toggle2CustomSettings").checked,numbers:document.getElementById("toggle3CustomSettings").checked,symbols:document.getElementById("toggle4CustomSettings").checked,separator:document.getElementById("toggle5CustomSettings").checked}}async function saveCustomProfile(label,settings){const id=generateSafeId(label);const entry={label:label,...settings};const allEncrypted=localStorage.getItem("carrypass_customTypesEnc");let all={};if(allEncrypted){try{all=await decryptWithSessionKey(allEncrypted)}catch(e){console.warn("Failed to decrypt custom profiles. Starting fresh.")}}const overwrite=all[id]!==undefined;const doSave=()=>{all[id]=entry;encryptWithSessionKey(all).then(enc=>{localStorage.setItem("carrypass_customTypesEnc",enc);loadCustomProfilesIntoSelector()})};if(overwrite){const msg=tWithVars("overwriteCustomProfile",{profileName:label});showConfirmation(msg,confirmed=>confirmed&&doSave())}else{doSave()}}async function encryptWithSessionKey(obj){if(!sessionKey)throw new Error("No session key available");const iv=crypto.getRandomValues(new Uint8Array(12));const encoded=(new TextEncoder).encode(JSON.stringify(obj));const ciphertext=await crypto.subtle.encrypt({name:"AES-GCM",iv:iv},sessionKey,encoded);return JSON.stringify({iv:Array.from(iv),cipher:Array.from(new Uint8Array(ciphertext))})}async function decryptWithSessionKey(blob){if(!sessionKey)throw new Error("No session key available");const{iv,cipher}=JSON.parse(blob);const plaintext=await crypto.subtle.decrypt({name:"AES-GCM",iv:new Uint8Array(iv)},sessionKey,new Uint8Array(cipher));return JSON.parse((new TextDecoder).decode(plaintext))}async function loadCustomProfilesIntoSelector(){const selector=document.getElementById("profileSelector");if(!selector)return;[...selector.options].forEach(opt=>{if(opt.dataset.dynamic==="true")opt.remove()});const raw=localStorage.getItem("carrypass_customTypesEnc");if(!raw)return;let customProfiles={};try{customProfiles=await decryptWithSessionKey(raw)}catch(e){console.warn("Could not decrypt custom profiles:",e);return}const keys=Object.keys(customProfiles);if(keys.length>0){const separator=document.createElement("option");separator.textContent=t("customProfilesSeparator");separator.disabled=true;separator.dataset.dynamic="true";selector.appendChild(separator)}for(const id of keys){const opt=document.createElement("option");opt.value=id;opt.textContent=customProfiles[id].label||id;opt.dataset.dynamic="true";selector.appendChild(opt)}}function ensureCustomProfileInSelector(name){const selector=document.getElementById("profileSelector");if(!selector||!name)return;const exists=[...selector.options].some(opt=>opt.value===name);if(!exists){const opt=document.createElement("option");opt.value=name;opt.textContent=name;opt.dataset.dynamic="true";selector.appendChild(opt)}}const PASSWORD_PROFILES={pin:{length:4,iterationCount:1e4,uppercase:false,lowercase:false,numbers:true,symbols:false,separator:false},pinsix:{length:6,iterationCount:1e4,uppercase:false,lowercase:false,numbers:true,symbols:false,separator:false},"short":{length:10,iterationCount:15e3,uppercase:false,lowercase:true,numbers:true,symbols:false,separator:false},medium:{length:21,iterationCount:3e4,uppercase:true,lowercase:true,numbers:true,symbols:false,separator:false},structured:{length:23,iterationCount:42e3,uppercase:true,lowercase:true,numbers:true,symbols:false,separator:true},"long":{length:51,iterationCount:7e4,uppercase:true,lowercase:true,numbers:true,symbols:true,separator:false},diceware:{length:4,iterationCount:5e4,uppercase:false,lowercase:true,numbers:false,symbols:false,separator:true},"diceware-short":{length:6,iterationCount:3e4,uppercase:false,lowercase:true,numbers:false,symbols:false,separator:true},"diceware-memorable":{length:5,iterationCount:4e4,uppercase:false,lowercase:true,numbers:false,symbols:false,separator:true},mixed:{length:21,iterationCount:3e4,uppercase:true,lowercase:true,numbers:true,symbols:true,separator:false}};function tuneIterationCount(baseCount,appMasterHex){const bytes=new Uint8Array(appMasterHex.match(/.{1,2}/g).map(b=>parseInt(b,16)));const hash=CryptoJS.SHA256(CryptoJS.enc.Hex.parse(appMasterHex)).toString();const pseudoRandom=parseInt(hash.slice(0,4),16);let min,max;if(baseCount<=2e4){min=1e3;max=2e4}else if(baseCount<=33333){min=21e3;max=33333}else if(baseCount<=66666){min=35e3;max=66e3}else{min=67e3;max=9e4}const range=max-min;const offset=Math.floor(pseudoRandom/65535*range);const finalCount=min+offset;return finalCount}async function collectInputData(){const variantCounter=parseInt(document.getElementById("counter").value.trim(),10)||0;let lengthInput,iterationCountInput,uppercaseChecked,lowercaseChecked,numbersChecked,symbolsChecked,separatorChecked;const profileSelected=document.getElementById("profileSelector")?.value;if(identity){const builtIns=["structured","medium","short","mixed","pin","pinsix","long","diceware","diceware-short","diceware-memorable"];if(!builtIns.includes(profileSelected)){try{const encryptedBlob=localStorage.getItem("carrypass_customTypesEnc");if(!encryptedBlob)throw new Error("No encrypted custom types found.");const customProfiles=await decryptWithSessionKey(encryptedBlob);const selectedProfile=customProfiles[profileSelected];if(selectedProfile){applyPasswordSettingsToUI(selectedProfile);lengthInput=selectedProfile.length??28;iterationCountInput=selectedProfile.iterationCount??100;uppercaseChecked=selectedProfile.uppercase??true;lowercaseChecked=selectedProfile.lowercase??true;numbersChecked=selectedProfile.numbers??true;symbolsChecked=selectedProfile.symbols??false;separatorChecked=selectedProfile.separator??true}else{showModalAlert("passwordTypesNotFound");return}}catch(err){console.error("Failed to decrypt custom profiles:",err);showModalAlert("passwordTypesLoad");return}}if(profileSelected&&PASSWORD_PROFILES[profileSelected]){const profile=PASSWORD_PROFILES[profileSelected];lengthInput=profile.length;iterationCountInput=profile.iterationCount;uppercaseChecked=profile.uppercase;lowercaseChecked=profile.lowercase;numbersChecked=profile.numbers;symbolsChecked=profile.symbols;separatorChecked=profile.separator;document.getElementById("length").value=profile.length;document.getElementById("iterationCount").value=profile.iterationCount;document.getElementById("toggle1").checked=profile.uppercase;document.getElementById("toggle2").checked=profile.lowercase;document.getElementById("toggle3").checked=profile.numbers;document.getElementById("toggle4").checked=profile.symbols;document.getElementById("toggle5").checked=profile.separator}}else{lengthInput=document.getElementById("length").value;iterationCountInput=document.getElementById("iterationCount").value;uppercaseChecked=document.getElementById("toggle1").checked;lowercaseChecked=document.getElementById("toggle2").checked;numbersChecked=document.getElementById("toggle3").checked;symbolsChecked=document.getElementById("toggle4").checked;separatorChecked=document.getElementById("toggle5").checked}const rawCount=parseInt(iterationCountInput,10);const isBuiltIn=PASSWORD_PROFILES.hasOwnProperty(profileSelected);const tunedIterationCount=identity&&isBuiltIn?tuneIterationCount(rawCount,appMaster):rawCount;const button=document.getElementById("collectInput");const webAddressField=document.getElementById("webAddress");const passwordField=document.getElementById("password");if(!webAddressField.value.trim()){webAddressField.focus();return}if(!identity){if(!passwordField.value.trim()){passwordField.focus();return}if(passwordField.value.trim().length<12){showModalAlert("masterPasswordLength");passwordField.focus();return}}button.disabled=true;const webAddressInput=document.getElementById("webAddress").value;const passwordInput=identity?appMaster:document.getElementById("password").value.trim();resetTimer();let charTypes={uppercase:!!uppercaseChecked,lowercase:!!lowercaseChecked,numbers:!!numbersChecked,symbols:!!symbolsChecked};setTimeout(async()=>{await generateDeterministicPasswordsWithAESCTR(webAddressInput,passwordInput,lengthInput,tunedIterationCount,charTypes,separatorChecked,variantCounter);const storedPasswordIndex=parseInt(sessionStorage.getItem("carrypass_indexPassword"),10);const storedUsernameIndex=parseInt(sessionStorage.getItem("carrypass_indexUsername"),10);if(!isNaN(storedPasswordIndex)){currentPasswordIndex=storedPasswordIndex}else{currentPasswordIndex=0}updatePasswordIndexLabel(currentPasswordIndex);displayPasswordFromIndex?.(currentPasswordIndex);if(!isNaN(storedUsernameIndex)){currentUsernameIndex=storedUsernameIndex}else{currentUsernameIndex=0}updateUsernameIndexLabel(currentUsernameIndex);displayUsernameFromIndex?.(currentUsernameIndex);sessionStorage.removeItem("carrypass_indexPassword");sessionStorage.removeItem("carrypass_indexUsername");button.disabled=false;resetSingleFeedbackIcon()},10)}function deterministicInsert(str,char,seed){const hash=CryptoJS.SHA256(seed+char).words[0];const pos=Math.abs(hash)%str.length;return str.slice(0,pos)+char+str.slice(pos+1)}function segmentKeyFixedLength(password,groupSize=7,separator="-"){const chars=password.split("");const maxInsertions=Math.floor(password.length/(groupSize+1));for(let i=1;i<=maxInsertions;i++){const insertPos=i*groupSize+(i-1);if(insertPos<chars.length){chars[insertPos]=separator}}return chars.join("")}function mulberry32(seed){return function(){let t=seed+=1831565813;t=Math.imul(t^t>>>15,t|1);t^=t+Math.imul(t^t>>>7,t|61);return((t^t>>>14)>>>0)/4294967296}}async function generateDicewarePassphrase(masterKey,service,iterationCount,variant=0,wordListType="standard"){const BASE_WORD_COUNT=4;const wordCount=BASE_WORD_COUNT+variant;const saltInput=`diceware::${service}::${iterationCount}::${wordCount}::${wordListType}`;const salt=(new TextEncoder).encode(saltInput);const wordLists={standard:EFF_WORDS,"short":EFF_SHORT_WORDS,memorable:EFF_MEMORABLE_WORDS};const wordList=wordLists[wordListType]||EFF_WORDS;const wordListLength=wordList.length;const maxAcceptable=Math.floor(65536/wordListLength)*wordListLength;const hashLen=wordCount*4+16;const hash=await argon2.hash({pass:masterKey,salt:salt,mem:65536,time:2,hashLen:hashLen,type:argon2.ArgonType.Argon2id,raw:true});const bytes=new Uint8Array(hash.hash);const words=[];let i=0;while(words.length<wordCount&&i+1<bytes.length-8){const number=bytes[i]<<8|bytes[i+1];i+=2;if(number<maxAcceptable){const index=number%wordListLength;words.push(wordList[index])}}if(words.length<wordCount){const seedOffset=bytes.length-4;const seed=bytes[seedOffset]<<24|bytes[seedOffset+1]<<16|bytes[seedOffset+2]<<8|bytes[seedOffset+3];const rand=mulberry32(seed>>>0);while(words.length<wordCount){const r=Math.floor(rand()*65536);if(r<maxAcceptable){words.push(wordList[r%wordListLength])}}}return words.join(" ")}async function formatKeyPassRejectionSample(bytes,length,charset,rules=[],input=""){const charsetLength=charset.length;if(charsetLength<2)throw new Error("Charset too small");const result=[];const has={};const maxByte=256;const maxValid=Math.floor(maxByte/charsetLength)*charsetLength;const flatBytes=[];for(let i=0;i<bytes.words.length*4;i++){const byte=bytes.words[Math.floor(i/4)]>>24-i%4*8&255;flatBytes.push(byte)}let byteIndex=0;const seedMaterial=CryptoJS.SHA256(input+flatBytes.slice(0,32).join(",")).toString();const seedWords=CryptoJS.enc.Hex.parse(seedMaterial).words;const seed=seedWords[0]>>>0;const rand=mulberry32(seed);while(result.length<length){let byte;if(byteIndex<flatBytes.length){byte=flatBytes[byteIndex++]}else{byte=Math.floor(rand()*256)}if(byte>=maxValid)continue;const index=byte%charsetLength;const char=charset.charAt(index);result.push(char);rules.forEach(rule=>{if(!(rule in has)&&new RegExp(rule).test(char)){has[rule]=result.length-1}})}rules.forEach(rule=>{if(!(rule in has)){let insertChar="?";if(rule==="[A-Z]")insertChar="G";if(rule==="[a-z]")insertChar="g";if(rule==="[0-9]")insertChar="4";if(rule==="[!@#$%^&*()\\-_=+\\[\\]{}|;:,.<>?]")insertChar="!";let insertPos=input.charCodeAt(0)%result.length;let attempts=0;while(rules.some(r=>has[r]===insertPos)&&attempts<result.length){insertPos=(insertPos+1)%result.length;attempts++}result[insertPos]=insertChar}});return result.join("")}function formatKeyPass(bytes,length,charset,rules=[],input=""){let result="";const has={};const positions=[];for(let i=0;i<bytes.words.length*4;i++){const byte=bytes.words[Math.floor(i/4)]>>24-i%4*8&255;const char=charset.charAt(byte%charset.length);result+=char;positions.push(char);rules.forEach(rule=>{if(!has[rule]&&new RegExp(rule).test(char)){has[rule]=i}});if(result.length>=length)break}result=result.slice(0,length).split("");rules.forEach(rule=>{if(!(rule in has)){let insertChar="?";if(rule==="[A-Z]")insertChar="G";if(rule==="[a-z]")insertChar="g";if(rule==="[0-9]")insertChar="4";if(rule==="[!@#$%^&*()\\-_=+\\[\\]{}|;:,.<>?]")insertChar="!";let insertPos=input.charCodeAt(0)%result.length;let safetyCounter=0;while(rules.some(r=>has[r]===insertPos)&&safetyCounter<result.length){insertPos=(insertPos+1)%result.length;safetyCounter++}result[insertPos]=insertChar}});return result.join("")}const PASSWORDCHARSETS={upperAlpha:"ZJVEQSLBWRGNMYXDOCHIPATUKF",lowerAlpha:"pvjaqukxhyngorfizwmtlbdecs",numbers:"72940538615784216903",special:"!@#&$%*^()_+[]{}|;:,.<>?/"};function getEffectivePBKDF2Iterations(userInput){const parsed=parseInt(userInput);const input=isNaN(parsed)||parsed<1?1:parsed;let iterations;if(input>66666){iterations=Math.floor((1e5+input)/3)}else if(input>33333){iterations=Math.floor((5e4+input)/3)}else{iterations=Math.floor((2e4+input)/3)}return Math.max(1e3,iterations)}function colorizePassword(password){return password.split("").map(char=>{if(/[a-z]/.test(char))return`<span class="lower">${char}</span>`;if(/[A-Z]/.test(char))return`<span class="upper">${char}</span>`;if(/[0-9]/.test(char))return`<span class="digit">${char}</span>`;if(/[^a-zA-Z0-9]/.test(char))return`<span class="symbol">${char}</span>`;return char}).join("")}function finalizePasswordGenerationUI(service){document.getElementById("serviceLabel").innerHTML="<b>"+service+"</b>";document.getElementById("webAddress").value="";document.getElementById("password").value="";document.getElementById("password").disabled=true;const dot=document.getElementById("dot");dot.style.backgroundColor="#e7f1ff";updateCounter();resetTimer()}async function generateDeterministicPasswordsWithAESCTR(webAddress,masterPassword,length,iterationCount,charTypes,segmented=false,variant=0){const normalizedService=normalizeServiceName(webAddress);const profileSelected=document.getElementById("profileSelector")?.value;const isDicewareProfile=profileSelected?.startsWith("diceware");let wordListType="standard";if(profileSelected==="diceware-short")wordListType="short";else if(profileSelected==="diceware-memorable")wordListType="memorable";if(isDicewareProfile){const passwords=[];for(let i=0;i<6;i++){const pw=await generateDicewarePassphrase(masterPassword,normalizedService+"::"+i,iterationCount,variant,wordListType);passwords.push(pw)}updateGeneratedPasswords(passwords);generateUsernames();finalizePasswordGenerationUI(webAddress);const generatedCard=document.getElementById("passwordCard");showSmooth(generatedCard);return}let charset="";if(charTypes.uppercase)charset+=PASSWORDCHARSETS.upperAlpha;if(charTypes.lowercase)charset+=PASSWORDCHARSETS.lowerAlpha;if(charTypes.numbers)charset+=PASSWORDCHARSETS.numbers;if(charTypes.symbols)charset+=PASSWORDCHARSETS.special;if(!charset){showModalAlert("selectMinCharType");return[]}const variantPart=identity?`::variant=${variant}`:"";const versionTag=identity?"::v2":"";const charsetSignature=identity?"::charset="+[...new Set(charset)].sort().join(""):"";const saltInputArgon=normalizedService+"::"+iterationCount+"::"+length+variantPart+charsetSignature+versionTag;const argonSalt=(new TextEncoder).encode(saltInputArgon);const options=mapArgonOptions(iterationCount);const argonHash=await argon2.hash({pass:masterPassword,salt:argonSalt,...options,hashLen:32,raw:true});const argonEnhancer=Array.from(argonHash.hash).map(b=>b.toString(16).padStart(2,"0")).join("");const saltInput=normalizedService+"::"+iterationCount+"::"+length+variantPart+charsetSignature+versionTag+"::"+argonEnhancer;const salt=CryptoJS.SHA256(saltInput).toString();const key=CryptoJS.PBKDF2(masterPassword,salt,{keySize:256/32,iterations:getEffectivePBKDF2Iterations(iterationCount),hasher:CryptoJS.algo.SHA256});const passwords=[];for(let index=0;index<6;index++){let stream=CryptoJS.lib.WordArray.create();let blockCounter=0;const requiredBytes=identity?length*3:length*2;while(stream.words.length*4<requiredBytes){const ivSource=identity?CryptoJS.HmacSHA256(`pwgen::${normalizedService}::${index}::${blockCounter}`,key).toString():CryptoJS.SHA256(`pwgen::${normalizedService}::${index}::${blockCounter}`).toString();const iv=CryptoJS.enc.Hex.parse(ivSource.substring(0,32));const inputBlock=CryptoJS.enc.Hex.parse("00000000000000000000000000000000");const encrypted=CryptoJS.AES.encrypt(inputBlock,key,{mode:CryptoJS.mode.CTR,iv:iv,padding:CryptoJS.pad.NoPadding});stream=CryptoJS.lib.WordArray.create(stream.words.concat(encrypted.ciphertext.words));blockCounter++}const rules=[];if(charTypes.uppercase)rules.push("[A-Z]");if(charTypes.lowercase)rules.push("[a-z]");if(charTypes.numbers)rules.push("[0-9]");if(charTypes.symbols)rules.push("[!@#$%^&*()\\-_=+\\[\\]{}|;:,.<>?]");let password=identity?await formatKeyPassRejectionSample(stream,length,charset,rules,normalizedService):formatKeyPass(stream,length,charset,rules,normalizedService);if(segmented){password=segmentKeyFixedLength(password,7,"-")}passwords.push(password)}updateGeneratedPasswords(passwords);generateUsernames();const generatedCard=document.getElementById("passwordCard");showSmooth(generatedCard);document.getElementById("serviceLabel").innerHTML="<b>"+webAddress+"</b>";document.getElementById("webAddress").value="";document.getElementById("password").value="";document.getElementById("password").disabled=true;const dot=document.getElementById("dot");dot.style.backgroundColor="#e7f1ff";updateCounter();resetTimer()}function mapArgonOptions(iterationCountInput){const parsed=parseInt(iterationCountInput);const input=isNaN(parsed)?1:parsed;let time=2;let mem=65536;if(input>66666){time=5;mem=196608}else if(input>33333){time=3;mem=131072}const parallelism=1;return{time:time,mem:mem,parallelism:parallelism,type:argon2.ArgonType.Argon2id}}document.addEventListener("DOMContentLoaded",()=>{const webAddress=document.getElementById("webAddress");const password=document.getElementById("password");if(!webAddress||!password)return;webAddress.addEventListener("input",()=>{if(webAddress.value.trim().length>0){password.disabled=false}else{password.disabled=true;password.value=""}})});document.addEventListener("DOMContentLoaded",()=>{const toggle1=document.getElementById("toggle1");const toggle2=document.getElementById("toggle2");const toggle3=document.getElementById("toggle3");const toggle4=document.getElementById("toggle4");const trackedToggles=[toggle1,toggle2,toggle3,toggle4];trackedToggles.forEach(toggle=>{toggle.addEventListener("change",()=>{const checkedCount=trackedToggles.filter(t=>t.checked).length;if(checkedCount===0){toggle.checked=true;const container=toggle.closest(".toggle-container");if(container){container.classList.add("bounce");setTimeout(()=>{container.classList.remove("bounce")},300)}}})})});function tWithVars(key,vars={}){const lang=localStorage.getItem("preferredLanguage")||"en";let template=translations[lang]?.[key]||key;for(const[k,v]of Object.entries(vars)){template=template.replace(new RegExp(`\\$\\{${k}\\}`,"g"),v)}return template}function applyPasswordSettingsToUI(settings){if(!settings)return;const lengthInput=document.getElementById("length");const iterationInput=document.getElementById("iterationCount");const toggleUpper=document.getElementById("toggle1");const toggleLower=document.getElementById("toggle2");const toggleNums=document.getElementById("toggle3");const toggleSyms=document.getElementById("toggle4");const toggleSep=document.getElementById("toggle5");if(lengthInput)lengthInput.value=settings.length??28;if(iterationInput)iterationInput.value=settings.iterationCount??100;if(toggleUpper)toggleUpper.checked=settings.uppercase??true;if(toggleLower)toggleLower.checked=settings.lowercase??true;if(toggleNums)toggleNums.checked=settings.numbers??true;if(toggleSyms)toggleSyms.checked=settings.symbols??false;if(toggleSep)toggleSep.checked=settings.separator??true}async function decryptCustomProfiles(){const raw=localStorage.getItem("carrypass_customTypesEnc");if(!raw)return{};try{return await decryptWithSessionKey(raw)}catch(err){console.warn("Custom profile decryption failed:",err);return{}}}function buildSavedServicesList(){const listContainer=document.getElementById("savedServicesList");listContainer.innerHTML="";for(const serviceName of cachedServiceList){const li=document.createElement("li");li.style.display="flex";li.style.alignItems="center";li.style.justifyContent="space-between";li.style.marginBottom="0.5rem";const link=document.createElement("a");link.href="#toggleSavedServices";link.textContent=serviceName;link.style.marginRight="1rem";link.style.color="#1E003B";link.style.textDecoration="none";link.style.flexGrow="1";link.addEventListener("click",async e=>{e.preventDefault();document.getElementById("webAddress").value=serviceName;const passwordField=document.getElementById("password");if(passwordField)passwordField.disabled=false;const settings=cachedServiceMap[serviceName]||{};await loadCustomProfilesIntoSelector();const allProfiles=await decryptCustomProfiles();if(settings.passwordType){ensureCustomProfileInSelector(settings.passwordType);const profileSelector=document.getElementById("profileSelector");if(profileSelector)profileSelector.value=settings.passwordType;const selectedProfile=allProfiles[settings.passwordType];if(selectedProfile){applyPasswordSettingsToUI(selectedProfile)}}const variantInput=document.getElementById("counter");if(variantInput&&typeof settings.variant==="number"){variantInput.value=settings.variant}sessionStorage.setItem("carrypass_indexPassword",settings.passwordIndex??0);sessionStorage.setItem("carrypass_indexUsername",settings.usernameIndex??0);document.getElementById("webAddress").dispatchEvent(new Event("blur"));const panel=document.getElementById("savedServicesPanel");if(panel){panel.classList.remove("open")}});const trashWrapper=document.createElement("button");trashWrapper.style.background="none";trashWrapper.style.border="none";trashWrapper.style.cursor="pointer";trashWrapper.style.padding="0";trashWrapper.style.marginLeft="0.5rem";trashWrapper.style.display="flex";trashWrapper.style.alignItems="center";trashWrapper.setAttribute("aria-label","Delete service");const trashIcon=document.createElement("i");trashIcon.setAttribute("data-lucide","trash-2");trashIcon.style.width="20px";trashIcon.style.height="20px";trashIcon.style.color="#842029";trashWrapper.appendChild(trashIcon);trashWrapper.addEventListener("click",event=>{event.stopPropagation();event.preventDefault();const message=tWithVars("deleteServiceConfirm",{service:serviceName});showConfirmation(message,async confirmed=>{if(confirmed){const index=cachedServiceList.indexOf(serviceName);if(index!==-1)cachedServiceList.splice(index,1);delete cachedServiceMap[serviceName];settingsDirty=true;await saveServiceSettingsBlobs();buildSavedServicesList();const manageButton=document.getElementById("toggleSavedServices");if(manageButton){manageButton.classList.add("import-delete-flash");setTimeout(()=>{manageButton.classList.remove("import-delete-flash")},1e3)}}})});li.appendChild(link);li.appendChild(trashWrapper);listContainer.appendChild(li)}if(typeof lucide!=="undefined"){lucide.createIcons()}}let pendingDeleteService=null;function showDeleteModal(serviceName){const modal=document.getElementById("confirmationModal");const modalMessage=document.getElementById("confirmationMessage");pendingDeleteService=serviceName;modalMessage.innerHTML=`
